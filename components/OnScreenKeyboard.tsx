...
> = ({ children, onClick, className = "", flex = 1 }) => (
  <button
    ...
    className={`h-10 tall:h-14 rounded-md font-semibold text-gray-800 flex items-center justify-center bg-gray-200 shadow-[0_4px_0_0_#9ca3af] active:shadow-[0_1px_0_0_#9ca3af] active:translate-y-1 active:bg-gray-300 transition-all ${className}`}
    style={{ flex: `${flex}` }}
  >
    {children}
  </button>
...
> = ({ children, onClick, disabled }) => (
  <button
    onClick={onClick}
    disabled={disabled}
    className="h-10 w-full rounded-md font-semibold text-white bg-blue-500 hover:bg-blue-600 disabled:bg-gray-400 disabled:cursor-not-allowed text-sm"
  >
    {children}
  </button>
);
...
const OnScreenKeyboard: React.FC<OnScreenKeyboardProps> = ({
  ...
  useEffect(() => {
    if (ref.current) {
      console.log(
        `%c[OnScreenKeyboard] Rendered size: ${ref.current.offsetHeight}px`,
        "color: #e67e22"
      );
    }
  });
  ...
  return (
    <div
      ref={ref}
      className="w-full max-w-lg mx-auto p-1 bg-gray-200/90 rounded-lg select-none"
    >
      {isHintView ? (
        ...
        <div className="flex flex-col gap-0.5 tall:gap-1.5">
